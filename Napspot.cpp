...
void Napspot::add_reservation(const string& napspot_id, const string& time) {
    ...
    /*if (!db->query("reservations", "napspot_id", "start_time", time).empty()) {
        cerr << "Reservation already made" << endl;
        return false;
    }
     */
    //cerr << db->query("reservations", "reservation_id", "start_time", time) << endl;
